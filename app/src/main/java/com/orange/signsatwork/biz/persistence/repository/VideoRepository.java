...
public interface VideoRepository extends CrudRepository<VideoDB, Long> {
    ...
    @Query(value="select a.sign_id, b.name, a.create_date, a.id, a.url, a.picture_uri, a.nb_view, a.average_rate, a.nb_comment, a.id_for_name, b.nb_video from videos a inner join signs b inner join favorites_videos c inner join communities_videos d inner join communities_users e  on a.sign_id = b.id and c.videos_id = a.id and c.favorites_id = :favoriteId  and a.id = d.videos_id and d.communities_id = e.communities_id and e.users_id = :userId union select a.sign_id, b.name, a.create_date, a.id, a.url, a.picture_uri, a.nb_view, a.average_rate, a.nb_comment, a.id_for_name, b.nb_video from videos a inner join signs b inner join favorites_videos c on a.sign_id = b.id and c.videos_id = a.id and c.favorites_id = :favoriteId and a.user_id = :userId order by create_date desc", nativeQuery = true)
