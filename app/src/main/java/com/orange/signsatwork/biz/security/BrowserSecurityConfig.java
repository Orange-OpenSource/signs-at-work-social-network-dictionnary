...
public class BrowserSecurityConfig extends WebSecurityConfigurerAdapter {
  ...
  private void configureRoutes(HttpSecurity http) throws Exception {
    ...
    http.csrf().disable()
            ...
            .authorizeRequests()
            // open api is... opened
            .antMatchers("/cgu", "/sendMail", "/forgetPassword", "/user/createPassword*", "/user/changePassword*", "/user/*/savePassword").permitAll()
//            .antMatchers("/", "/signs/**", "/sign/**").permitAll()
  ...
  private void disableSecurityOnAssets(HttpSecurity http) throws Exception {
    http.authorizeRequests().antMatchers("/img/**", "/font/**","/files/**", "/.well-known/**", "/manifest/**").permitAll();
  }
