...
echo $errorMessage
	...
	if [ "$4" != null ] && [ "$signId" != null ]; then
		url=http://localhost:8080/ws/sec/signs/$signId
echo $url
		createDefinitionSignResponse=$(curl -s --noproxy '*' -u "$3" -H 'Content-Type: multipart/form-data' -X PUT -F 'data={"textDefinition":"'"$4"'"};type=application/json' $url)
...
echo $errorMessage
	...
	if [ "$5" != null ] && [ "$signId" != null ]; then
		url=http://localhost:8080/ws/sec/signs/$signId
echo $url
		createDefinitionSignResponse=$(curl -s --noproxy '*' -u "$3" -H 'Content-Type: multipart/form-data' -X PUT -F 'data={"textDefinition":"'"$5"'"};type=application/json' $url)
		echo $createDefinitionSignResponse
        fi
...
function create_favorite() {
	...
	url=http://localhost:8080/ws/sec/favorites
	createFavorite=$(curl -s --noproxy '*' -u "$2" --header 'Content-Type: application/json' --request POST --data '{"name":"'"$1"'"}' $url)
	echo $createFavorite
...
function add_sign_to_favorite() {
	...
	echo user $2
	echo videoId $3
	url=http://localhost:8080/ws/sec/favorites/$1
	addSignToFavorite=$(curl -s --noproxy '*' -u "$2" --header 'Content-Type: application/json' --request PUT --data '{"videoIdToAdd":"'"$3"'"}' $url)
...
echo $list_name_encode
	url=http://localhost:8080/ws/sec/users/me/favorites?name="$list_name_encode"
echo $url
